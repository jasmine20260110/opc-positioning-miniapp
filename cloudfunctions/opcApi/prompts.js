...
请以JSON格式输出：
{
  "routes":[{
    ...
    "launch_requirements":{
      "first_revenue_cycle":"保守月份区间，如1—3个月","first_revenue_amount_range":"保守金额区间，如100—1000元",
      "minimum_weekly_time":"每周小时数，如5—7小时","minimum_validation_budget":"最低金额，如0—500元",
      "first_validation_users":{"count":"人数或团队数，如3—5人","channel":"一个具体可执行渠道"}
    }
  }]
}
规则：routes必须正好对应A、B、C；市场卡只能包含上述5项；没有购买、咨询或访谈证据时需求强度不得为强；不生成无来源的市场规模、转化率、精确收入或客户评价；市场证据无法判断时写待验证；第三步不读取个人启动条件、不做路线排序、不输出JSON之外的文字。
launch_requirements是用于低成本试跑的“Demo保守启动门槛假设”，只用于比较启动难度，不是已验证市场事实。因此5个可比较字段禁止填写待验证，但不得把这些数字描述为真实行业门槛。周期统一用阿拉伯数字和“个月”，金额统一用阿拉伯数字和“元”，时间统一用阿拉伯数字和“小时”，用户数使用阿拉伯数字和“人”或“个团队”；使用保守整数或区间，不使用虚假精确小数。
启动要求必须分别根据每条路线的目标用户、最小验证动作、获客渠道和商业化路径推导，禁止给A/B/C复制同一组要求。A、B、C在变现周期、金额、每周时间、验证预算、首批用户数这5项组成的组合必须各不相同；渠道也必须具体可执行，不能只写待验证。
`.trim();
...
请以JSON格式输出：
{
  ...
  "plan_note":"说明计划基于当前信息、关键假设仍待验证，并给出风险提示和执行边界",
  "days":[{
    "day":1,
    "goal":"当天唯一目标",
...
3. 7天内至少包含一次真实用户或市场验证，并至少测试一次明确的付费信号；Day 7根据证据决定继续、调整或暂停；
4. 每天必须包含预计用时、完成证据和降级方案；单日预计用时必须为15—60分钟，7天预计用时上限之和不能超过用户每周可投入时间上限；
5. plan_note必须明确说明计划基于当前信息、关键假设仍待验证；
6. 不建议辞职、借贷、大额投入或不可逆行动；信息不足时用“待验证”，不得编造市场数据和用户反馈；
7. 只输出JSON，不要输出Markdown或JSON之外的文字。
`.trim();
